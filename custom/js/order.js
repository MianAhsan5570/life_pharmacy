	   	...
	   	for (var x = 0; x < quantity.length; x++) {       						
			...
			if(orderDate && clientName && clientContact && paid && discount && paymentType && paymentStatus) {
				if(validateProduct == true && validateQuantity == true) {
	            	...
	            	'<a href="orders.php?o=editOrd&i='+response.order_id+'" class="btn btn-default" style="margin-left:10px;"> <i class="glyphicon glyphicon-plus-sign"></i> Edit Order </a>'+
...
//print order with stock minius
function printOrder2(orderId = null) {
	if(orderId) {		
...
function getTotal(row = null) {
	if(row) {
		...
		totalp = Number(total)/100;
		totalp1 = totalp*Number($("#percentage"+row).val());
		total = total-totalp1;
		total = total.toFixed(2);
		$("#total"+row).val(total);
		$("#totalValue"+row).val(total);
...
function subAmount() {
	...
	// sub total
	$("#subTotal").val(Number(totalSubAmount)+Number(totaldiscount));
	$("#subTotalValue").val(Number(totalSubAmount)+Number(totaldiscount));

	// vat
	var vat = (Number($("#subTotal").val())/100) * 5;
	vat = vat.toFixed(2);
	$("#vat").val(totaldiscount);
	$("#vatValue").val(totaldiscount);
