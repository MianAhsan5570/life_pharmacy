...
if($_POST) {	
  ...
  //  $fetchCustomer =mysqli_fetch_assoc(mysqli_query($dbc,"SELECT * FROM customers WHERE customer_id='$clientName'"));
	...
	$sql = "INSERT INTO purchase (purchase_date, client_name, client_contact, sub_total, vat, total_amount, discount, grand_total, paid, due ) VALUES ('$purchaseDate', '$clientName', '$clientContact', '$subTotalValue', '$vatValue', '$totalAmountValue', '$discount', '$grandTotalValue', '$paid', '$dueValue' )";
	...
	for($x = 0; $x < count($_POST['productName']); $x++) {			
		...
		while ($updateProductQuantityResult = $updateProductQuantityData->fetch_row()) {
			...
			$rate1now = $_POST['rate1'][$x];
			$alert_at = $_POST['alert'][$x];

				// update product table
				$updateProductTable = "UPDATE product SET purchase='$rate_2',alert_at='$alert_at', quantity = '".$updateQuantity[$x]."' WHERE product_id = ".$_POST['productName'][$x]."";
				$connect->query($updateProductTable);

				// add into order_item
				 $orderItemSql = "INSERT INTO purchase_item (purchase_id, product_id, quantity, rate, total, purchase_item_status,pack,pack_quantity,discount,expiry_date,packrate) 
				VALUES ('$_SESSION[order_id]', '".$_POST['productName'][$x]."', '".$_POST['quantity'][$x]."', '".$_POST['rate'][$x]."', '".$_POST['totalValue'][$x]."', 1,'".$_POST['pack'][$x]."',
			'".$_POST['pac_quantity'][$x]."',
		'".$_POST['totalpercentage'][$x]."',
		'".$_POST['expiry'][$x]."',
		'".$_POST['purchase_box'][$x]."'
