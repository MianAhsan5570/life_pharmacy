	...
	if(row) {
		...
		if(productId == "") {
			...
			$.ajax({
				...
				type: 'post',
				data: {productId : productId, type : 'purchase'},
				dataType: 'json',
				success:function(response) {
					...
					$("#rate1"+row).val(response.rate);
					$("#alert"+row).val(response.alert_at);
					$("#purchase_value"+row).val(response.rate);
					$("#quantity_show"+row).val(response.quantity);
					$("#quantity"+row).val(1);
					$("#purchase_box"+row).val(response.packrate);
					$("#pac_quantity"+row).val(response.pack_quantity);
					$("#totalpercentage"+row).val(response.discount);
...
function getTotal(row = null) {
	if(row) {
		...
		var total = Number($("#purchase_box"+row).val()) * Number($("#pack"+row).val());
		var totalqty = Number($("#pack"+row).val()) * Number($("#pac_quantity"+row).val());
		var getpurchaserate = Number($("#totalpercentage"+row).val());
		var actualpurchasetate =  Number($("#purchase_box"+row).val()) / Number($("#pac_quantity"+row).val());
		
		var boxrate = total; // pack x purchase rate
		 var totaldics = boxrate/100;
		 var totaldicsnow = totaldics*getpurchaserate;
		 //alert(totaldicsnow);

		 total = total-totaldicsnow;
	var purchaseValue = Number($("#purchase_box" + row).val());
var quantityValue = Number($("#pac_quantity" + row).val());
var totalPercentageValue = Number($("#totalpercentage" + row).val());

var PurchaseActua0 = purchaseValue / quantityValue;
var PurchaseActua1 = (PurchaseActua0 / 100) * totalPercentageValue;
var PurchaseActua = PurchaseActua0 - PurchaseActua1;

		total = total.toFixed(2);
		$("#total"+row).val(total);
		$("#totalValue"+row).val(total);
		$("#quantity"+row).val(totalqty);
		$("#purchase"+row).val(PurchaseActua);
		$("#rate1"+row).attr("min", actualpurchasetate);
		// $('#expiry'+row).val(expiry);
