...
$(document).ready(function () {
	...
	manageCategoriesTable = $('#manageCategoriesTable').DataTable({
		processing: true,
		serverSide: true,
		ajax: { url: 'php_action/fetchCategories.php', type: 'POST', data: function (d) { return d; } },
		order: [[0, 'asc']],
		pageLength: 20,
	...
	$('#addCategoriesModalBtn').unbind('click').bind('click', function () {
		...
		$("#submitCategoriesForm").unbind('submit').bind('submit', function () {
			...
			var categoriesqty = $("#categoriesqty").val();
	...
	if (categoriesId) {
		...
		$.ajax({
			...
			success: function (response) {
				...
				$("#editCategoriesqty").val(response.low_stock);
				$("#editCategoriesidnnew").val(response.categories_id);
				...
				$("#editCategoriesForm").unbind('submit').bind('submit', function () {
					...
					var editCategoriesqty = $("#editCategoriesqty").val();
					var editCategoriesId = $("#editCategoriesId").val();

					if (editCategoriesqty == "") {
						$("#editCategoriesqty").after('<p class="text-danger">Brand Name field is required</p>');
						$('#editCategoriesqty').closest('.form-group').addClass('has-error');
					} else {
						// remov error text field
						$("#editCategoriesqty").find('.text-danger').remove();
						// success out for form 
						$("#editCategoriesqty").closest('.form-group').addClass('has-success');
					}
